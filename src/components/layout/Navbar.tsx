...
import { ShoppingBag, Heart, Search, Menu, X, Leaf, User, LogOut, ChevronDown, Package } from "lucide-react";
  ...
  return (
    <>
      <nav className={cn(
        ...
        <div className="container-custom">
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="flex items-center gap-1 md:gap-2">
              <button onClick={() => setSearchOpen(!searchOpen)} className="btn-ghost p-2 w-10 h-10" aria-label="Search">
                <Search className="w-5 h-5" />
              </button>
              <Link to="/shop?wishlist=true" className="btn-ghost p-2 w-10 h-10 relative" aria-label="Wishlist">
                <Heart className="w-5 h-5" />
                {wishlistCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-rose-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">{wishlistCount}</span>
                )}
              </Link>
              <Link to="/cart" className="btn-ghost p-2 w-10 h-10 relative" aria-label="Cart">
                <ShoppingBag className="w-5 h-5" />
                {totalItems > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-gold-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">{totalItems}</span>
                )}
              </Link>
              ...
              {user ? (
                <div className="relative hidden md:block" ref={userMenuRef}>
                  ...
                  >
                    ...
                    <ChevronDown className={cn("w-3.5 h-3.5 text-muted-foreground transition-transform hidden lg:block", userMenuOpen && "rotate-180")} />
                  </button>

                  {/* Dropdown */}
                  {userMenuOpen && (
                    <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl shadow-product border border-border overflow-hidden animate-fade-in z-50">
                ...
                <Link
                  to="/login"
                  className="hidden md:flex items-center gap-2 btn-outline py-2 px-4 text-sm"
                >
              ...
              <button onClick={() => setMobileOpen(!mobileOpen)} className="btn-ghost p-2 w-10 h-10 md:hidden" aria-label="Menu">
        ...
        {/* Search Bar */}
        {searchOpen && (
          <div className="border-t border-border bg-white/95 backdrop-blur-md animate-fade-in">
        ...
        {mobileOpen && (
          <div className="md:hidden border-t border-border bg-white/95 backdrop-blur-md animate-fade-in">
            <div className="container-custom py-6 flex flex-col gap-4">
              {navLinks.map((l) => (
                <Link key={l.to} to={l.to} className={cn(
                  "font-sans text-base font-medium py-2 transition-colors",
                  location.pathname === l.to ? "text-gold-600" : "text-foreground"
                )}>{l.label}</Link>
              ))}
              ...
              <div className="pt-4 border-t border-border space-y-3">
                {user ? (
                  <>
                    <div className="flex items-center gap-3 py-2">
                      <div className="w-8 h-8 rounded-full bg-gold-gradient flex items-center justify-center flex-shrink-0 overflow-hidden">
                        {user?.avatar ? (
                          <img src={user.avatar} alt={user.username} className="w-full h-full object-cover" />
                        ) : (
                    ...
                    <Link to="/account" className="flex items-center gap-2 font-sans text-sm text-foreground hover:text-gold-600 transition-colors py-2">
                      <Package className="w-4 h-4" /> My Account & Orders
                    </Link>
                    <button
                      onClick={handleLogout}
                      className="flex items-center gap-2 font-sans text-sm text-rose-500 hover:text-rose-700 transition-colors py-2"
                    >
                ...
                <Link to="/admin" className="font-sans text-xs text-muted-foreground hover:text-gold-600 transition-colors">Admin Dashboard</Link>
              </div>
            </div>
          </div>
        )}
